...
public class BookService {
    ...
    private final BookDAO bookDAO;      //final로 만들면 객체 생성이 안된다.
    ...
    *   2. 순환참조를 스프링 시작(컨테이너 생성)과 동시에 확인하고 에러가 있을 시 에러를 발생시켜 준다.
    ...
    *   4. 테스트 코드 작성이 용이하다.(mock 객체 주입이 용이하다.)*/

    // 생성자 주입을 하면 에러가 발생 시 처음부터 생겨서 빠른 대응이 가능하지만 , 필드와 세터참조는 중간부터 에러가 터져서 힘들다
    @Autowired
